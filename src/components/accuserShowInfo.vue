<template>
  ...
  <fieldset class="layui-elem-field layui-field-title" style="margin-top: 25px" id="judge_inquiry_after_defendant_reply">
    <legend>审判员询问</legend>
    <div class="layui-card">
      <div class="layui-card-body">
        <form class="layui-form layui-form-pane">
          <template v-for="(item, index) in data.judge_inquiry_after_defendant_query" :key="index">
            <div class="layui-form-item" style="margin-bottom: 10px" pane>
              <label class="layui-form-label">审判员</label>
              <div class="layui-input-block" >
                <input type="text" v-model="data.judge_inquiry_after_defendant_query[index].question" placeholder="请输入问题" autocomplete="off"
                       class="layui-input" style="width: 100%;float: left;">
              </div>
            </div>
            <form class="layui-form">
              <template v-for="(aitem, aindex) in data.judge_inquiry_after_defendant_query[index].answer" :key="aindex">
                <div class="layui-form-item" pane>
                  <div class="layui-inline" style="width: 100%;margin-bottom:0px;">
                    <div class="layui-input-inline" style="margin-left:0px;margin-top:31px;" >
                      <VueMultiselect :option-height="38"  :show-labels="false" v-model="data.judge_inquiry_after_defendant_query[index].answer[aindex].name"
                                      :options="getAccuserMergeDefendant" placeholder="请选择当事人"
                                      style="line-height: 16px;width: 210px; min-height: 38px"></VueMultiselect>
                    </div>
                    <div class="layui-input-block">
                      <div class="myselect-div">
                      <textarea type="text" v-model="data.judge_inquiry_after_defendant_query[index].answer[aindex].answer" placeholder="请输入回答" autocomplete="off"
                                class="layui-textarea"  style="width: 85%;float: left;min-height:100px"></textarea>
                        <button type="button" @click="delete_component('judge_inquiry_after_defendant_query',index,aindex)" class="layui-btn layui-btn-danger layui-btn-sm"
                                data-type="text"
                                style="float: right;height:100px;width:7.5%">
                          <i class="layui-icon">&#xe640;</i></button>
                        <button type="button" @click="add_component('judge_inquiry_after_defendant_query',index,aindex+1)" class="layui-btn layui-btn-sm" data-type="text"
                                style="float: right;height:100px;width:7.5%">
                          <i class="layui-icon">&#xe654;</i>
                        </button>
                      </div>
            ...
            <div class="layui-form-item" style="width:100%;margin-top: -10px;">
              <button type="button" class="layui-btn layui-btn-radius layui-btn-xs" @click="add_component('judge_inquiry_after_defendant_info',index+1)"> 添加问题</button>
              <button type="button" class="layui-btn layui-btn-radius layui-btn-danger layui-btn-xs" style="margin-left: 5px" @click="delete_component('judge_inquiry_after_defendant_info',index)"> 删除问题
              </button>
            </div>
          </template>
        </form>
...
export default {
  ...
  data() {
    ...
    data = {
      ...
      judge_inquiry_after_defendant_query: [
        {
          question: "",
          answer: [
            {
  ...
  methods: {
    ...
    add_component(datatype, index, QAindex) {
      switch (datatype) {
        ...
        default:
          //这里是没有找到对应的值处理
          break;
        case "judge_inquiry_after_defendant_query":
          this.data.judge_inquiry_after_defendant_query[index].answer.splice(QAindex,0,{
            name:"",
            answer:"",
          });
          break;
        case "judge_inquiry_after_defendant_info":
          this.data.judge_inquiry_after_defendant_query.splice(index,0,{
            question: "",
            answer: [{
              name:"",
              answer:"",
    ...
    delete_component(datatype, index, QAindex) {
      switch (datatype) {
        ...
        case "judge_inquiry_after_defendant_query":
          //这里是值对应的处理
          if (this.data.judge_inquiry_after_defendant_query[index].answer.length < 2) {
            window.layer.msg("不允许删除唯一项", {icon: 5, time: 1500});
            return;
          }
          this.data.judge_inquiry_after_defendant_query[index].answer.splice(QAindex, 1);
          break;
        case "judge_inquiry_after_defendant_info":
          //这里是值对应的处理
          if (this.data.judge_inquiry_after_defendant_query.length < 2) {
            window.layer.msg("不允许删除唯一项", {icon: 5, time: 1500});
            return;
          }
          this.data.judge_inquiry_after_defendant_query.splice(index, 1);
          break;
        default:
          //这里是没有找到对应的值处理
          break;
